...
namespace ProtoType
{
    public partial class HalfYearlyColumn : System.Web.UI.Page
    {
        ...
        public DataTable GetDepartmentDowntime3(string rdeptCode)
        {
            try
            {
                ...
                string query = "SELECT function_desc, equip_desc, SUM(duration_min) AS dur FROM MAIN_DATA_R WHERE dept_code = @rdeptCode AND start_date >= DATEADD(MONTH,-6,GETDATE()) AND start_date <= GETDATE() GROUP BY function_desc, equip_desc WITH ROLLUP HAVING (function_desc IS NOT NULL AND equip_desc IS NOT NULL) OR (function_desc IS NOT NULL AND equip_desc IS NULL)";
                cmd = new SqlCommand(query, con);
        ...
        public DataTable GetDepartmentDowntime4(string eqclass)
        {
            try
            {
                ...
                string query = "SELECT m.function_desc, m.equip_desc, SUM(m.duration_min) AS dur FROM MAIN_DATA_R m, equipmenttb e where m.start_date >= DATEADD(MONTH,-6,GETDATE()) AND m.start_date <= GETDATE() AND m.equip_code = e.equip_code AND e.equip_class = @eqclass GROUP BY m.function_desc, m.equip_desc WITH ROLLUP HAVING (m.function_desc IS NOT NULL AND m.equip_desc IS NOT NULL) OR (m.function_desc IS NOT NULL AND m.equip_desc IS NULL)";
                cmd = new SqlCommand(query, con);
        ...
        public DataTable GetEquipmentDowntime(string department)
        {
            try
            {
                ...
                string query = "SELECT m.equip_desc, SUM(m.duration_min) AS dur FROM MAIN_DATA_R m WHERE m.function_desc = '" + department + "' AND m.equip_desc IS NOT NULL AND m.start_date >= DATEADD(MONTH,-6,GETDATE())  AND m.start_date <= GETDATE() GROUP BY m.equip_desc";
        ...
        public DataTable GetEquipmentDowntime2(string department, string maintenanceCode)
        {
            try
            {
                ...
                string query = "SELECT m.equip_desc, SUM(m.duration_min) AS dur FROM MAIN_DATA_R m WHERE m.function_desc = @department AND m.maint_code=@maintenanceCode AND m.equip_desc IS NOT NULL AND m.start_date >= DATEADD(MONTH,-6,GETDATE())  AND m.start_date <= GETDATE() GROUP BY m.equip_desc";
                cmd = new SqlCommand(query, con);
        ...
        public DataTable GetEquipmentDowntime3(string department, string rdeptCode)
        {
            try
            {
                ...
                string query = "SELECT m.equip_desc, SUM(m.duration_min) AS dur FROM MAIN_DATA_R m WHERE m.function_desc = @department AND m.dept_code = @rdeptCode AND m.equip_desc IS NOT NULL AND m.start_date >= DATEADD(MONTH,-6,GETDATE())  AND m.start_date <= GETDATE() GROUP BY m.equip_desc";
                cmd = new SqlCommand(query, con);
                cmd.Parameters.AddWithValue("@department", department);
                cmd.Parameters.AddWithValue("@rdeptCode", rdeptCode);
                adp = new SqlDataAdapter(cmd);
        ...
        public static List<GraphModel> ShowDept()
        {
            ...
            HalfYearlyColumn obj = (HalfYearlyColumn)HttpContext.Current.Handler;
        ...
        {
            Response.Redirect("StdReportHalfYear.aspx");
        }
    }
}